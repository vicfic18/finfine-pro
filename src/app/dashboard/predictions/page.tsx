...
export default function PredictiveModelingPage() {
  ...
  const fetchAdvisory = async (sector: string) => {
    try {
      ...
      const res = await authenticatedFetch(`/api/dashboard/predictions/advisory?sector=${encodeURIComponent(sector)}`);
      if (res.ok) {
  ...
  const loadDataAndSettings = async () => {
    try {
      ...
      void fetchAdvisory(currentSector);
    } catch (err) {
  ...
  const handleSectorSelectAndSave = async (sector: string) => {
    ...
    try {
      await authenticatedFetch('/api/dashboard/settings', {
        method: 'PUT',
  ...
  const handleRefreshAdvisory = async () => {
    try {
      setAdvisoryLoading(true);
      const res = await authenticatedFetch('/api/dashboard/predictions/advisory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
