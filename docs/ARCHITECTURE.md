...
* **Amazon Bedrock (Claude 3.5 Haiku - Vision Mode):** Acts as the multimodal extraction engine. By executing structured schema extraction directly against invoice and statement images, it bypasses the language limitations of traditional OCR engines, capturing multilingual terminology, non-standard invoice formats, and handwritten receipts at low token costs.
* **AWS Lambda (Ingestion Normalizer):** Validates the raw JSON emitted by the vision model against normalized system structures. It resolves dates into standardized formats, extracts statutory identifiers (GSTIN, PAN), reconciles transaction classifications, and executes batch writes to DynamoDB.
